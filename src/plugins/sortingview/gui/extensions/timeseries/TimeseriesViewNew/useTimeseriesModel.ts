...
const getTimeseriesDataSegment = async (args: {kacheryNode: KacheryNode | undefined, recordingObject: any, ds_factor: number, segment_num: number, segment_size: number, channelName: ChannelName, backendId: string | null}): Promise<number[][] | undefined> => {
  const { kacheryNode, recordingObject, ds_factor, segment_num, segment_size, channelName, backendId } = args
...
const useTimeseriesData = (recordingObject: any, recordingInfo: RecordingInfo): TimeseriesData | null => {
  ...
  const fetch = useMemo(() => (async (query: TimeseriesDataQuery) => {
    switch(query.type) {
      case 'dataSegment': {
        return await getTimeseriesDataSegment({kacheryNode, recordingObject, ds_factor: query.ds_factor, segment_num: query.segment_num, segment_size: query.segment_size, channelName, backendId})
      }
    }
  }), [kacheryNode, recordingObject, channelName, backendId])
