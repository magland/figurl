...
const getSnippetsInfo = async (args: {recording: Recording, sorting: Sorting, unitId: number | number[], kacheryNode: KacheryNode, channelName: ChannelName, backendId: string | null}): Promise<InfoType> => {
    const { recording, sorting, unitId, kacheryNode, channelName, backendId } = args
...
const getSnippets = async (args: {recording: Recording, sorting: Sorting, unitId: number | number[], timeRange: {min: number, max: number}, kacheryNode: KacheryNode, channelName: ChannelName, backendId: string | null}): Promise<Snippet[]> => {
    const { recording, sorting, unitId, timeRange, kacheryNode, channelName, backendId } = args
    const result = await runPureCalculationTaskAsync<{
        channel_ids: number[]
        channel_locations: number[][]
        sampling_frequency: number
        ...
        async (query: QueryType) => {
            switch(query.type) {
                case 'info':
                    ...
                    return await getSnippetsInfo({recording: query.recording, sorting: query.sorting, unitId: uid1, kacheryNode, channelName, backendId})
                case 'snippets':
                    const uid2 = applyMergesToUnit(query.unitId, curation, selection.applyMerges)
                    return await getSnippets({recording: query.recording, sorting: query.sorting, unitId: uid2, timeRange: query.timeRange, kacheryNode, channelName, backendId})
            }
        }
    ), [kacheryNode, channelName, backendId, curation, selection.applyMerges])
